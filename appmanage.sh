...
配置文件以冒号分割为三列，第一列表示容器的名字，第二列表示容器内tomcat应用的路径，第三列表示容器内tomcat程序的路径，第四列表示应用的名称; \n\
如果应用部署在宿主机，则删除第一个冒号前面的部分即可，（冒号不可省略），例如: \n\

# 这是备注或说明行 \n\
container_name:/app/path:/app/path/webapps:appname \n\
:/app/path:/app/path/webapps:appname \n\

第一行：可在配置文件开头、结尾、中间中添加以#号开头的备注或说明行，但不可在配置项同一行后面添加#号及说明 \n\
第二行：容器名为container_name，容器内应用的路径为/app/path，项目路径为/app/path/webapps，应用的名称为appname(该列可任意命名,但不能有重复的名称) \n\
...
docker exec -w /tmp $CONTAINERNAME tar -cvzPf ${ALIASAPPNAME}_${DATEDAY}.tar.gz $APPDATAPATH
docker cp $CONTAINERNAME:/tmp/${ALIASAPPNAME}_${DATEDAY}.tar.gz $BACKUPDIR
docker exec $CONTAINERNAME rm -rf /tmp/${ALIASAPPNAME}_${DATEDAY}.tar.gz
echo "备份已完成，备份文件位置:$BACKUPDIR/${ALIASAPPNAME}_${DATEDAY}.tar.gz"
}

nocontainer_backup () {

tar -cvzPf $BACKUPDIR/${ALIASAPPNAME}_${DATEDAY}.tar.gz $APPDATAPATH 
echo "备份已完成，备份文件位置:$BACKUPDIR/${ALIASAPPNAME}_${DATEDAY}.tar.gz"
...
sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}'
echo 
...
if [ $? -eq 0 ]
then
    ALIASAPPNAME="`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}' | sed 's/ //g' | awk -F: 'NR == '$appnum' {print $2}'`"
    APPPATH="`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '$4 ~ /'$ALIASAPPNAME'/ {print $2}'`"
    APPDATAPATH="`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '$4 ~ /'$ALIASAPPNAME'/ {print $3}'`"
    APPNAME="`echo $APPPATH | xargs basename`"
    CONTAINERNAME="`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '$4 ~ /'$ALIASAPPNAME'/ {print $1}'`"
    echo
    echo -e "1. 重启该应用;\n\
2. 停止该应用;\n\
3. 备份该应用的数据文件;\n\
4. 查看该应用的日志;"
    echo
    echo -e "你选择需要操作的应用是:`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}' | sed 's/ //g' | awk -F: 'NR == '$appnum' {print $2}'`,请继续选择你的动作:\c" 
    read actionnum
    case $actionnum in
    1)
        echo -e "你希望重启的应用是: `sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}' | sed 's/ //g' | awk -F: 'NR == '$appnum' {print $2}'`,你确信要重启这个应用吗([Y/y] ro [N/n]):\c"
        read YORN
    ...
    2)
    echo -e "你希望停止的应用是: `sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}' | sed 's/ //g' | awk -F: 'NR == '$appnum' {print $2}'`,你确信要停止这个应用吗([Y/y] ro [N/n]):\c"
    read YORN
    if [ x"$YORN" = x"Y" -o x"$YORN" = x"y" ]
    then
        if [ -z "$CONTAINERNAME" ]
    ...
    3)
    echo -e "现在将要备份你选定的应用: `sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$4}' | sed 's/ //g' | awk -F: 'NR == '$appnum' {print $2}'`,默认备份在 $BACKUPDIR 目录下..."
    if [ -z "$CONTAINERNAME" ]
    then
