...
echo "---------------------------------此脚本从应用服务器$APPSERVER下载文件,并上传到$FTPSITE的脚本,`date "+%Y-%m-%d_%H:%M:%S"`:up_bams_send_small_prize.sh----------------------"  >> $SCRIPTDIR/up_bams_send_small_prize.log
echo "登录系统 Server,检查Server服务器上的$APPSERVERFLAG目录下是否有标志文件,将文件列表存入本地$SCRIPTDIR/bams_send_small_prize_flag_up.txt文件中" >> $SCRIPTDIR/up_bams_send_small_prize.log
check_flag >> $SCRIPTDIR/up_bams_send_small_prize.log 2>&1
cd $SCRIPTDIR
if [ -s $SCRIPTDIR/bams_send_small_prize_flag_up.txt ]
then
    echo "$APPSERVER的$APPSERVERFLAG目录下存在标志文件,现在将Server上$APPSERVERFILE目录下的相应文件下载到本地..." >> $SCRIPTDIR/up_bams_send_small_prize.log
    for i in  `cat $SCRIPTDIR/bams_send_small_prize_flag_up.txt`
    do
        cd $SCRIPTDIR
        getfiletolocal $i
        BUSI="`echo $i | awk -F[_] '{print $2}'`"
        echo "将$i文件压缩后通过FTP上传到$FTPSITE服务器的$REMOTE_UPLOAD_DIR/$BUSI目录下..." >> $SCRIPTDIR/up_bams_send_small_prize.log
        zip  ${i}.zip  $i  >> $SCRIPTDIR/up_bams_send_small_prize.log
        ftp_up_file  $REMOTE_UPLOAD_DIR/$BUSI  ${i}.zip
        echo "备份$i文件到本地$BACKUPDIR目录中..."  >> $SCRIPTDIR/up_bams_send_small_prize.log
        tar rvf $BACKUPDIR/up_bams_cash_${DATEFORMAT}.tar $i >> $SCRIPTDIR/up_bams_send_small_prize.log  2>&1
        echo "删除$APPSERVER服务器$APPSERVERFLAG目录下的标志文件..." >> $SCRIPTDIR/up_bams_send_small_prize.log
        delete_flag  $i
