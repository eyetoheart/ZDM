...
echo "---------------------------------此脚本为从FTP Server下载文件的脚本,`date "+%Y-%m-%d_%H:%M:%S"`:funds_down_test.sh----------------------"  >> $SCRIPTDIR/logs_ftpdown.log
...
then
    ...
    do
        ...
        ftp_down_file  $REMOTE_DOWN_DIR/$BUSI  $i
        echo "将$i文件上传到$APPSERVER服务器的$APPSERVERFILE目录下..." >> $SCRIPTDIR/logs_ftpdown.log
        upfiletoapp  $i
