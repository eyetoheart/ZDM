...
#############################################
## file name : check_upload_test.sh
## creator:zhangdm
## create time:2017-04-07
## modify time:2017-04-14
...
cd $SCRIPTDIR
echo "open $FTPSITE" > $SCRIPTDIR/test3_check_up_ftptask.txt
echo "user $FTPUSER $FTPPASS" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "verbose" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "binary" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "passive" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "cd $1" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "mput $2" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "close" >> $SCRIPTDIR/test3_check_up_ftptask.txt
echo "bye" >> $SCRIPTDIR/test3_check_up_ftptask.txt
/usr/bin/ftp -vin < $SCRIPTDIR/test3_check_up_ftptask.txt >> $SCRIPTDIR/logs_ftpup.log 2>&1
rm $SCRIPTDIR/test3_check_up_ftptask.txt
...
if [ -s $SCRIPTDIR/flag_up.txt ]
then
    echo "$APPSERVER的$APPSERVER_CHECK_FUNDS_RESULT_FLAG目录下存在标志文件,现在将Server上$APPSERVER_CHECK_FUNDS_RESULT_FILE目录下的相应文件下载到本地..." >> $SCRIPTDIR/logs_ftpup.log
    for i in  `cat $SCRIPTDIR/flag_up.txt`
    do
        echo "get data:$i" >> $SCRIPTDIR/logs_ftpup.log
