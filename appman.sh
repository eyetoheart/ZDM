...
第二行：容器名为container_name，容器内应用的路径为/app/path，应用的名称为appname(该列可任意命名) \n\
...
then
    ...
    then
        ...
        kill -9 $uidnum
        sleep 5
        cd $APPPATH/work ; rm -rf ./*
        cd $APPPATH/temp ; rm -rf ./*
        cd $APPPATH/bin
...
then
    ...
    then
        ...
        docker exec $CONTAINERNAME $APPPATH/bin/startup.sh > /dev/null 2>&1
        sleep 3
...
echo -e "请选择你想重启的应用前面的数字:\c"
read inputnum
/usr/bin/seq 1 $configfilelinenumber | grep $inputnum > /dev/null 2>&1
if [ $? -eq 0 ]
then
    echo -e "你希望重启的应用是: `sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$3}' | awk -F ': ' 'NR == '$inputnum' {print $2}'`,你确信要重启这个应用吗([Y/y] ro [N/n]):\c"
    read YORN
    if [ x"$YORN" = x"Y" -o x"$YORN" = x"y" ]
    then
        ALIASAPPNAME="`sed '/^$/d;/^#/d' $PROGCONF | awk -F: '{print NR,":",$3}' | awk -F ': ' 'NR == '$inputnum' {print $2}'`"
