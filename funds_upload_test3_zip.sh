...
#############################################
## file name : funds_upload_test.sh
## creator:zhangdm
...
cd $SCRIPTDIR
echo "open $FTPSITE" > $SCRIPTDIR/test3_up_ftptask.txt
echo "user $FTPUSER $FTPPASS" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "verbose" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "binary" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "passive" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "cd $1" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "mput $2" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "close" >> $SCRIPTDIR/test3_up_ftptask.txt
echo "bye" >> $SCRIPTDIR/test3_up_ftptask.txt
/usr/bin/ftp -vin < $SCRIPTDIR/test3_up_ftptask.txt >> $SCRIPTDIR/logs_ftpup.log 2>&1
rm $SCRIPTDIR/test3_up_ftptask.txt
...
echo "---------------------------------此脚本为应用服务器$APPSERVER下载文件,并上传到系统$FTPSITE的脚本,`date "+%Y-%m-%d_%H:%M:%S"`:funds_upload_test.sh----------------------"  >> $SCRIPTDIR/logs_ftpup.log
echo "登录系统 Server,检查Server服务器上的$APPSERVERFLAG目录下是否有标志文件,将文件列表存入本地$SCRIPTDIR/flag_up.txt文件中" >> $SCRIPTDIR/logs_ftpup.log
