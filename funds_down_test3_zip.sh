...
echo "---------------------------------此脚本为从FTP Server下载文件的脚本,`date "+%Y-%m-%d_%H:%M:%S"`:funds_down_test.sh----------------------"  >> $SCRIPTDIR/funds_down_test3_zip.log
echo "登录系统 Server,检查Server服务器上的$APPSERVERFLAG目录下是否有标志文件,将文件列表存入本地$SCRIPTDIR/flag_down.txt文件中" >> $SCRIPTDIR/funds_down_test3_zip.log
check_flag >> $SCRIPTDIR/funds_down_test3_zip.log 2>&1
cd $SCRIPTDIR
if [ -s $SCRIPTDIR/flag_down.txt ]
then
    echo "$APPSERVER的$APPSERVERFLAG目录下存在标志文件,现在根据此标志文件通过FTP登陆到$FTPSITE下载文件" >> $SCRIPTDIR/funds_down_test3_zip.log
    ...
    do
        ...
        if [ -f ${i}.zip ]
        then
            echo "将${i}.zip文件解压,并上传到$APPSERVER服务器的$APPSERVERFILE目录下..." >> $SCRIPTDIR/funds_down_test3_zip.log
            unzip  ${i}.zip >> $SCRIPTDIR/funds_down_test3_zip.log
            upfiletoapp  $i
            echo "在$APPSERVER服务器的$APPSERVERDOWNFLAG目录下创建标志文件..." >> $SCRIPTDIR/funds_down_test3_zip.log
            creat_flag  $i
            echo "删除$APPSERVER服务器$APPSERVERFLAG目录下的标志文件..." >> $SCRIPTDIR/funds_down_test3_zip.log
            delete_flag  $i
            echo "备份$i文件到本地$BACKUPDIR目录中..."  >> $SCRIPTDIR/funds_down_test3_zip.log
